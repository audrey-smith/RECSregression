...
recs09 <- read.csv('oldRECSdata/recs2009_public.csv')
recs05 <- read.csv('oldRECSdata/RECS05alldata.csv')
```
...
recsBrr05 <- recs05[,grepl('^BRRWT', colnames(recs05))]
recsBrrCol05 <- colnames(recsBrr05)
```
...
- [unsure what to do about ordinal categorical variables - there are merits to treating them as either numeric or factor. I think Min left them as numeric based on the fact that they later squared some of them]
```{r}
...
minSHrecs$heathomeYN <- as.factor(ifelse(minSHrecs$HEATHOME == 1, 1, 0))
...
minSHrecs$fuelheatElec <- as.factor(ifelse(minSHrecs$FUELHEAT == 5, 1, 0))
...
#minSHrecs$MONEYPY <- as.ordered(minSHrecs$MONEYPY)
#minSHrecs$EQUIPAGE <- as.ordered(minSHrecs$EQUIPAGE)
#minSHrecs$WINDOWS <- as.ordered(minSHrecs$WINDOWS)
#minSHrecs$TYPEHUQ <- as.ordered(minSHrecs$TYPEHUQ)

#Generating ln for response variable due to skewness
minSHrecs$logKWH <- log(minSHrecs$KWHSPH)

ggplot(data = minSHrecs) + geom_histogram(aes(x = log(TOTALBTUSPH+.00001)))
```
...
minSHmodel <- svyglm(formula = log(TOTALBTUSPH+.00001) ~ heathomeYN + HHAGE + yearmade40 + yearmade50 + yearmade60 + yearmade70 + TOTSQFT_EN + HDD65 + hddSq + MONEYPY + multifamYN + TEMPHOME + TEMPGONE + divisionWNC + divisionENC + equipageSq + fuelheatOil + fuelheatWood + fuelheatElec + fuelheatOther + walltypeBrick + walltypeWood + urbanYN + blackYN + WINDOWS, design = minSHdes, family = 'gaussian')
