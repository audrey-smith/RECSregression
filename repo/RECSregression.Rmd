...
-This one doesn't have division and removes redundancies in climate data
fullModel <- svyglm(formula = log(BTUNGSPH+.00001) ~ ELWARM + CLIMATE_COLD + CLIMATE_MARINE + CLIMATE_HOTHUMID + CLIMATE_MIXHUMID + UATYP10_U + UATYP10_C+ TYPEHUQ + YEARMADERANGE + TOTROOMS + HOUSEHOLDER_RACE_2 + HOUSEHOLDER_RACE_3 + HOUSEHOLDER_RACE_4 + HOUSEHOLDER_RACE_5 + HOUSEHOLDER_RACE_7 + SDESCENT + HHSEX_2 + EMPLOYHH + EDUCATION + MONEYPY + KOWNRENT_1 + ELPAY_1, design = recsDes, family = 'gaussian')
...
R2_heatNG <- with(summary(acsModel_heatNG), 1 - deviance/null.deviance)
...
acsModel_heatLPG <- svyglm(formula = log(BTULPSPH+.00001) ~ WDWARM + CLIMATE_MARINE + IECC_CLIMATE_PUB_78 + HDD65 + UATYP10_U + UATYP10_C+ TYPEHUQ + YEARMADERANGE + TOTROOMS + LPGPAY_1, design = recsDes, family = 'gaussian')

R2_heatLPG <- with(summary(acsModel_heatLPG), 1-deviance/null.deviance)
print(paste('R2:', R2_heatLPG, sep = ' '))
summary(acsModel_heatLPG)
acsModel_heatLPG
```

```{r}
#Space heating - fuel oil/kerosene
acsModel_heatFO <- svyglm(formula = log(BTUFOSPH+.00001) ~ FOWARM + CLIMATE_COLD + CLIMATE_MARINE + CLIMATE_MIXHUMID + IECC_CLIMATE_PUB_12A + IECC_CLIMATE_PUB_78 + HDD65 + UATYP10_U + TYPEHUQ + YEARMADERANGE + TOTROOMS + HOUSEHOLDER_RACE_7 + SDESCENT + FOPAY_1, design = recsDes, family = 'gaussian')

R2_heatFO <- with(summary(acsModel_heatFO), 1-deviance/null.deviance)
print(paste('R2:', R2_heatFO, sep = ' '))
summary(acsModel_heatFO)
acsModel_heatFO
```

Wood is a very poor fit and we do not have the same variables we have for others. Due to the outcome variables available, need to re-work into one regression for all wood use.
```{r}
#Space heating - wood
acsModel_heatWood <- svyglm(formula = log(WOODBTU + .00001) ~ ELWARM + CLIMATE_COLD + CLIMATE_MARINE + CLIMATE_HOTHUMID + CLIMATE_MIXHUMID + IECC_CLIMATE_PUB_12A + IECC_CLIMATE_PUB_6AB + IECC_CLIMATE_PUB_78 + HDD65 + UATYP10_U + UATYP10_C+ TYPEHUQ + YEARMADERANGE + TOTROOMS + HOUSEHOLDER_RACE_2 + HOUSEHOLDER_RACE_3 + HOUSEHOLDER_RACE_4 + HOUSEHOLDER_RACE_5 + HOUSEHOLDER_RACE_7 + SDESCENT + HHSEX_2 + EMPLOYHH + EDUCATION + MONEYPY + KOWNRENT_1, design = recsDes, family = 'gaussian')

R2_heatWood <- with(summary(acsModel_heatWood), 1-deviance/null.deviance)
print(paste('R2: ', R2_heatWood))
summary(acsModel_heatWood)
acsModel_heatWood
```

__General Space Heating__
```{r}
acsModel_sphTot <- svyglm(formula = log(TOTALBTUSPH + .00001) ~ HEATHOME + CLIMATE_COLD + CLIMATE_HOTHUMID + CLIMATE_MIXHUMID + IECC_CLIMATE_PUB_12A + IECC_CLIMATE_PUB_6AB + HDD65 + UATYP10_U + UATYP10_C+ TYPEHUQ + YEARMADERANGE + TOTROOMS + HOUSEHOLDER_RACE_2 + HOUSEHOLDER_RACE_7 + MONEYPY, design = recsDes, family = 'gaussian')

R2_sphTot <- with(summary(acsModel_sphTot), 1-deviance/null.deviance)
print(paste('R2:', R2_sphTot, sep = ' '))
summary(acsModel_sphTot)
acsModel_sphTot
```
...
```{r}
tractData <- read_sf('spatial/acsBuildAm_join.shp') %>%
  as.data.frame() %>%
  rename('popTot'='EstimatesT', 'white'='EstimatesW', 'black'='EstimatesB', 'amInd'='EstimatesA', 'asian'='Estimate_1', 'hiPI'='EstimatesN', 'other'='EstimatesS', 'multi'='EstimatesM', 'medIncome'='EstimatesM', 'unitsTot'='Estimates_3', 'unitsOccupied'='EstimatesO', 'unitsVacant'='EstimatesV', 'rooms1'='Estimates1', 'rooms2'='Estimates2', 'rooms3'='Estimates3', 'rooms4'='Estimates4', 'rooms5'='Estimates5', 'rooms6'='Estimates6', 'rooms7'='Estimates7', 'rooms8'='Estimates8', 'rooms9plus'='Estimates9')
```
