...
#Check Against EIA
predict_allLog %>%
  mutate(BTUelec = BTUelec*TOTALHH, BTUng = BTUng*TOTALHH, BTUlpg = BTUlpg*TOTALHH, BTUfo = BTUfo*TOTALHH, BTUwood = BTUwood*TOTALHH) %>%
  mutate(BTUtract = (BTUelec + BTUng + BTUlpg + BTUfo + BTUwood)) %>%
  group_by(state_code) %>%
  summarize(stateTotal = sum(BTUtract), stateElec = sum(BTUelec), stateNG = sum(BTUng), stateLPG = sum(BTUlpg), stateFO = sum(BTUfo), stateWd = sum(BTUwood)) %>%
  mutate(stateExpected = expected, propUnder = (stateExpected-stateTotal)/stateExpected)
```
...
```{r}
predict_allLog %>%
  mutate(BTUelec_scaleHH = BTUelec*TOTALHH, BTUng_scaleHH = BTUng*TOTALHH, BTUlpg_scaleHH = BTUlpg*TOTALHH, BTUfo_scaleHH = BTUfo*TOTALHH, BTUwd_scaleHH = BTUwood*TOTALHH) %>%
  mutate(BTUelec_scalePop = BTUelec*TOTALPOP, BTUng_scalePop = BTUng*TOTALPOP, BTUlpg_scalePop = BTUlpg*TOTALPOP, BTUfo_scalePop = BTUfo*TOTALPOP, BTUwd_scalePop = BTUwood*TOTALPOP) %>%
  write.csv('output/output_May29_byPopAndHH.csv')
```
